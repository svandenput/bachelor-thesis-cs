...
To prove theorem 1, we prove the cases N1, N2 and N3 separately. The full proof can be found in appendix B.
...
In this thesis, the only the N-schemes evaluated are alterations on the ones proven secure by NRS. NRS finds 20 possible N-schemes. Without using AD, 10 possible N-schemes remain. To more thoroughly investigate the security of different lAE compositions, the other 7 N-schemes should also be proven secure or be proven insecure with a counterexample.

\paragraph*{Adding AD}
The lAE construction is only evaluated without the possibility to add associated data. To allow for this, A slight modification should be made to the definition of the lAE. With AD added, there will be 20 possible N-schemes, three of witch will relate to the secure schemes from NRS. These three schemes should be prioritized as they have the highest likelihood of begin secure. For a more rigorous analysis, all 20 schemes should be evaluated.

\paragraph*{Evaluating lAE with multiple uses}
The evaluation of the lAE construction is limited to a key that is used once. A more in-depth analysis could evaluate the construction in a setting where a key can be used multiple times. In order to maintain a security, you would likely need to alter the lAE definition to also incorporate nonces. As this is a new input a new set of possible compositions should be generated and evaluated. 
...
To prove Theorem 1, we prove the cases N1, N2 and N3 separately, after proving all three we can conclude Theorem 1 holds.
\section{N1}
...
\begin{align*}
    ...
    \text{Pr}[\text{lAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
Next we define game N2-switch-1 in Figure \ref{fig: N2-switch-1 game}. The only difference between this game and game lAE-N2$^0$ is the fact that N2-switch-1 uses the uniformly random function $tag$, instead of the lMAC. To define this function we write $\mathit{Func}(\keyspace_{mac} \times \lockspace \times \ciphertextspace,\tagspace)$ to denote the set of all functions from the key space of the MAC \keyspace, the shared lock space \lockspace{} and ciphertext space from E.enc \ciphertextspace{} to the tag space \tagspace. We define this function specifically as we want the tags resulting from computations in oracle Oenc to match with those in oracle Odec. When the input of $tag$ is outside its domain, it will return \invalid.
