...
\section{Use Cases}
In this section, we will look at some use cases for the lAE. The most prominent use case is hybrid encryption as idea of locks originated in this setting. In addition to this, the primitive might be useful whenever a new key is generated often as this will increase the number of ephemeral keys used (and hence the chance on ephemeral key collision). Below we describe how the lAE can be used as a building block for hybrid encryption, as well as how it might be used in different settings.
...
The study of symmetric cryptographic primitives is usually done in a single-user setting, where one user that uses one key is considered. In reality, most systems have multiple users that are generate their own keys. To account for this, the notion of multi-user security, where you consider multiple users all generating their own keys, was first introduced by Bellare, Boldyreva and Micali \textbf{TODO: add citation}. Security bounds found in a single-user setting often degrade in a multi-user setting. To compensate for this degradation, one can expand the size of the key. Because this is not always feasible, GKP introduce locks as an alternative to key expansion. They show how ephemeral key collisions in a multi-user setting can lead to insecurities, as well as how you can augment security primitives with locks. Afterwards, a composition of an augmented MAC and an augmented DEM is shown be suitable for hybrid encryption.
\\\\
A similar construction to locks, called "id", is used to construct a sponge-based PRF \cite{EPRINT:BhaBhaNan22} and to construct a PFR out of a permutation \cite{EPRINT:kirby}. Just like the lock, this id is bound to the user to prevent degradation of security bounds due to user key collisions. The first construction assumes id to be unique while the second allows the id to be shared between multiple users. Both of the constructions are proven secure in a multi-user setting when the underlying primitives are secure.
\\\\
