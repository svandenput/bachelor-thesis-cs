...
\\\\
To prove this theorem we start by defining game loAE-N2 in figure \ref{fig: loAE-N2 game}. This game is the game loAE-IND-\$-CCA (figure \ref{fig: loAE-IND game}) with the security calls from N2 (figure \ref{fig: N2 calls}) filled in.
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
...
\begin{align*}
    ...
    \\& \hspace{0,3cm}  + \text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}}^0 =1]
    -
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =1]
\end{align*}
Any adversary \adversary{} winning game loAE-N2, but not N2-switch-1, can break the security of the loMAC as well. Concretely, we define adversary $B$ against loMAC that wins security game loMAC-PRF (figure \ref{fig: loMAC-PRF game}) using \adversary.
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
...
\noindent
This adversary has the runtime of \adversary{}. For every Oenc query \adversary{} makes, $B$ computes E.enc once, and call Omac once. For every Odec query \adversary{} makes, $B$ computes E.dec once and calls Ovrf once. Using $B$, we can see that $\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =1]
-
...
\noindent
In the real world, the oracle Odec from N2-switch-1 returns \invalid{} unless the condition on 24 is false. As a result, $\text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}}^0 =1] - \text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =1]$ will be equal to the chance of this condition being true. As \taginstance{} and \taginstance' are the result of the uniformly random function $tag$, where the input of $tag$ may not be equal to compute both, the chance of \taginstance{} and \taginstance' being equal will be $\frac{1}{\abs{t}^2}$. Combining this with at most $Q_d$ calls to the decryption oracle, we get:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
...
\end{align*}
In addition to this, games N2-switch-2 and loAE-N2 are equal in the ideal world, resulting in $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^1 =1]
-
