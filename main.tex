...
\begin{itemize}
    ...
    \begin{figure}[H]
        ...
        \end{pchstack}
        \caption{L-MIOT-IND game, \adversary{} has access to oracles Oenc and Odec and the tags in line 10 and 15 are the same.}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \end{pchstack}
        \caption{L-MIOT-UF game, \adversary{} has access to oracles Omac and Ovrf and the tags in line 11 and 16 are the same.}
    \end{figure}
\end{itemize}
...
\subsubsection{used primitives}
\begin{itemize}
    \item nE: A nonce-based E scheme is defined by triple (\keyspace{},E,D). E is a deterministic encryption algorithm that takes three inputs (K,N,M) to a value C, the length of C only depends the length of K, N and M. When (K,N,M) is not in $\keyspace \times \noncespace \times \messagespace$, C will be \invalid. D is the decryption algorithm that takes three inputs (K,N,C) to a value M. E and D are inverse of each other implying correctness (if E(K,N,M) $= C \neq \invalid$, then D(K,N,C) = M) and tidiness (if D(K,N,C) $= M \neq \invalid$, then E(K,N,M) = C). The security is defined as follows:\\ \fbox{\includegraphics[scale = 0.8]{images/sec nE.png}}
    \item MAC: The MAC is a deterministic algorithm F that takes in a K in \keyspace{} and a string x and outputs either a n-bit length T or \invalid. The domain of F is the set X such that F(K,x) $\neq \invalid$. The security of F is defined by \advantage{F}{pfr} =\probabilityblock{A}{}{F}{p}. the game on the left selects a random K from \keyspace{} and provides oracle access to F(K,.) the game on the right selects a uniformly random function p from X to \{1,0\}$^n$ and provide oracle access to it. With each oracle, queries outside X return \invalid.
\end{itemize}
