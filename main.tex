...
The part of \pkc{} that is of interest for us is the ADEM-then-AMAC construction. They start with a augmented data encapsulation mechanism, ADEM for short, that is secure against passive attackers. They combine this existing scheme with a augmented message authentication code, AMAC for short, to create a ADEM' that is secure against active attackers. The details of this are found below.

\subsubsection{notation}
\messagespace{} is a message space, \keyspace{} is a finite key space, \lockspace{} is a lock space and \ciphertextspace{} is a ciphertext space. \users{} is the number of users. The locks are called tags in \pkc, additionally, we call the output of the AMAC the tag instead of the ciphertext.
...
Below are the security notations from \recgc{} that are relevant for this thesis. Some of the variable names are rewritten towards a common notation with \pkc.
\subsubsection{notation}
...
We define the constructions secure as there is a tight reduction from breaking the nAE-security of the scheme to breaking the nE-security and the PRF security of the underlying primitives. Three different schemes, named N1, N2 and N3 were proven to be secure the can be viewed in \recgc{} figure 6.
