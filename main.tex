...
When writing out security notions, the limitations imposed on the adversary has can be written in a game based format. When doing this pseudocode is used instead of text. As a example, a security game for a generic encryption block using nonces in a multi-user setting is found in figure.


A bit that models the challenge is given to the game and the probability of the adversary solving the challenge is returned. For example, for IND-\$ the challenge bit will signal wether we are in the real of the ideal world and the chance of the adversary guessing this bit right is returned. In addition, the oracles that the adversary has access to area written out.
...
\paragraph{Security}
The security of a nAE is defined as \advantage{$\mathit{\Pi}$,\adversary}{nAE} = \probabilityblock{nAE-IND\$}{\adversary}{0}{1}, where nAE-IND\$ is in figure \ref{fig: nAE-IND game}. In this game, set $U$ keeps track of all used nonces as the adversary is not allowed to repeat those on encryption. Following the translation of IND\$ to a security game for AE from (\textbf{todo add citation for Automated Analysis of Protocols that use Authenticated Encryption: How Subtle AEAD}), \_ denotes a variable that is irrelevant and set $Q$ keeps tack of all query results in order to prevent trivial wins.
    \begin{figure}
        \centering
        \begin{pchstack}[boxed,center,space=0.5cm]
...
The security is defined as \advantage{\adversary,\users}{loAE} = \probabilityblock{loAE-IND\$}{\adversary,\users}{0}{1}, where loAE-IND\$ is in figure \ref{fig: loAE-IND game}.
\begin{figure}
    \begin{pchstack}[boxed,center,space=0.5cm]
