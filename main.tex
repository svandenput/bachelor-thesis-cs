...
\paragraph{N2}
We define two games, loAE-N2 and loAE-N2-switch.
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
...
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        \pseudocode[lnstart=-1,linenumbering,head={\textbf{Game} loAE-N2-switch$^{b}_{\adversary,\users}$ }]{
        L \result \emptyset\\
        \pcfor \user \in [1..N]:\\
        \t \keyinstance_\user \sample \keyspace\\
        \t C_\user \result \invalid\\
    ...
    \end{pchstack}
    \caption{loAE-N2-switch game, adversary{} has access to oracles Oenc and Odec. \textbf{abort} means the game stops running and returns 0.}
    \label{fig: loAE-N2-switch game}
\end{figure}
Any adversary \adversary playing game loAE-N2 that can obtain a output different from \invalid{} from the Odec query, can break the security of the loMAC as well. Concretely we build adversary $B$ against loMAC that breaks its sec game using adversary \adversary.
...
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        ...
        \pseudocode[lnstart=16,linenumbering,head={\text{if \adversary{} calls }\textbf{Oracle} Odec$(\user,\ciphertextinstance)$}]{
            ...
            (\ciphertextinstance',\taginstance) \result \ciphertextinstance\\
            \messageinstance \result \text{E.dec}(\keyinstance{}1,\lockinstance_j,\ciphertextinstance')\\
            t \result \text{Ovrf}(\user,\lockinstance_j,\ciphertextinstance')\\
            \pcif \taginstance \neq \taginstance' : \messageinstance \result \invalid\\
            \pcreturn \messageinstance
        }
    \end{pchstack}
...
\end{figure}
The probability that the condition in line 24 of loEA-N2 is false is exactly the probability that the condition in line 22 adversary B is false, and thus wins the loMAC game. As the two games only differ if this condition is false, we can state, regardless of b, that
\begin{align*}
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =1] - \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^0 =1]
    \leq
    \text{\advantage{$B$,\users}{loMAC}}
\end{align*}
In the switch game, no information can can ever be gained from making a Odec query. As a result, we can define a adversary $C$ that runs $A$, forwards all Oenc queries and disregards all Odec queries. This leads to:
\begin{align*}
    \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^0 =1] - \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^1 =1]
    =
    \text{\advantage{$C$,\users}{loE}}
\end{align*}
We can now prove our theorem:
\begin{align*}
\text{\advantage{$A$,\users}{loAE}}    &  = \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =1] - \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =1]
    \\& \leq \text{\advantage{$C$,\users}{loE}} + \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^0 =1] -  \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =1]
    \\& \leq \text{\advantage{$C$,\users}{loE}} + \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^0 =1] -  \text{Pr}[\text{loAE-N2-switch}_{\text{\adversary,\users}}^1 =1]
    \\& = \text{\advantage{$B$,\users}{loMAC}} + \text{\advantage{$C$,\users}{loE}}
\end{align*}
