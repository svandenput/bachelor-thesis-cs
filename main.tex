...
\begin{tabular}{|c | c | c | c | m{5cm}|}
    ...
    \hline
    ciphertext   &   $c$   &   $C$   &   \ciphertextspace & encrypted message \\
    \hline
...
\begin{itemize}
    \item ADEM: the ADEM exists of tuple (A.enc,A.dec), A.enc take a key K in \keyspace{}, a tag t in \tagspace{} and a message m in \messagespace{} and outputs a ciphertext c in \ciphertextspace{}. A.dec takes a K \keyspace{}, a tag t in \tagspace{} and a ciphertext c in \ciphertextspace{} and outputs a message m in \messagespace{} or $\bot$ to indicate rejection. The correctness requirement is that for all K in \keyspace{}  and t in \tagspace{} and m in \messagespace{} we have A.dec(K,t,A.enc(K,t,m)) = m. The security of the ADEM is defined with \advantage{ADEM,A,N}{n-moit-ind} = \probabilityblock{N-MIOT-IND}{A,N}{0}{1}, defined by the following game:
    \begin{figure}[H]
        \centering
        \includegraphics[scale = 0.7]{images/game adem.png}
    \end{figure}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale = 0.7]{images/game amac.png}
    \end{figure}
...
\noindent The AE schemes should be constructed from the DEM and the MAC. Following \gcrec{}, three ways to construct this AE are of interest, namely the ones following from the N1, N2 and N3 scheme. One thing to keep in mind with this that these schemes would originally use associated data. For now we can discard this but it is not proven that the same security results would also follow from this case without associated data. Down here the initial schemes can be found, followed by the AE.enc and AE.dec calls that can we construct following these schemes.
\begin{figure}[H]
    \centering
    \includegraphics[scale = 0.7]{images/N games.png}
\caption{Original N schemes from \gcrec}
\end{figure}

\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        \pseudocode[lnstart=-1,linenumbering,head={AE.enc(\keyinstance,\lockinstance,\messageinstance)}]{
            (k1,k2) \leftarrow \keyinstance \\
            \ciphertextinstance' = E.enc(k1,\lockinstance,\messageinstance) \\
            t = M.mac(k2,\lockinstance,\messageinstance) \\
            \ciphertextinstance = (\ciphertextinstance',t)\\
            \pcreturn \ciphertextinstance
        }
        \pseudocode[lnstart=4,linenumbering,head={AE.dec(\keyinstance,\lockinstance,\ciphertextinstance)}]{
            (k1,k2) \leftarrow \keyinstance \\
            (\ciphertextinstance',t) \leftarrow \ciphertextinstance \\
            \messageinstance = E.dec(k1,\lockinstance,\ciphertextinstance') \\
            t' = M.mac(k2,\lockinstance,\messageinstance) \\
            \pcif t = t' : \pcreturn \messageinstance \\
            \pcelse : \pcreturn \bot
        }
    \end{pchstack}
...
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        \pseudocode[lnstart=-1,linenumbering,head={AE.enc(\keyinstance,\lockinstance,\messageinstance)}]{
            (k1,k2) \leftarrow \keyinstance\\
            \ciphertextinstance' = E.enc(k1,\lockinstance,\messageinstance)\\
            t = M.mac(k2,\lockinstance,\ciphertextinstance')\\
            \ciphertextinstance = (\ciphertextinstance',t)\\
            \pcreturn \ciphertextinstance
        }
        \pseudocode[lnstart=4,linenumbering,head={AE.dec(\keyinstance,\lockinstance,\ciphertextinstance)}]{
...
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        \pseudocode[lnstart=-1,linenumbering,head={AE.enc(\keyinstance,\lockinstance,\messageinstance)}]{
            (k1,k2) \leftarrow \keyinstance\\
            t = M.mac(k2,\lockinstance,\messageinstance)\\
            \messageinstance' = \messageinstance || t\\
            \ciphertextinstance = E.enc(k1,\lockinstance,\messageinstance')\\
            \pcreturn \ciphertextinstance
        }
        \pseudocode[lnstart=4,linenumbering,head={AE.dec(\keyinstance,\lockinstance,\ciphertextinstance)}]{
            (k1,k2) \leftarrow \keyinstance\\
            \messageinstance' = E.dec(k1,\lockinstance,\ciphertextinstance)\\
            (\messageinstance,t) \leftarrow \messageinstance'\\
            t' = M.mac(k2,\lockinstance,\messageinstance)\\
            \pcif t = t' : \pcreturn \messageinstance \\
            \pcelse : \pcreturn \bot
        }
