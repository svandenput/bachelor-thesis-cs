...
\begin{align*}
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
To expand our probability again, we define game N2-switch-2 in Figure \ref{fig: N2-switch-2 game}. The Odec oracle from this game always returns \invalid{}, apart from that it is equal to the first switch game. The Odec oracle is written down more elaborately to include the event $bad$, this is to support prove tactics from \cite{EC:BelRog06}. We use this game to expand our probability:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &=
    \text{\advantage{$B$,\users}{loMAC}} 
...
\begin{align*}
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
Lastly, $\text{Pr}[\text{N2-switch-3}_{\text{\adversary,\users}} =0]$ and $\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0]$ are equal by definition, giving our result:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
