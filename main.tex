...
\begin{theorem}
Let loAE be constructed from loMAC and loE as described in Figure \ref{fig: N2 calls}. Assume ciphertext space \ciphertextspace{} from the loE to be a subset of message space \tagspace from the loMAC as well as loMAC and loE having a shared lock space. Then, for any number of users \users{} and any loAE adversary \adversary{} that poses at most $Q_e$ many Oenc queries, and at most $Q_d$ many Odec queries, there exist a loMAC adversary $B$ and a loE adversary $C$ such that:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}} \leq \text{\advantage{$B$,\users}{loMAC}} + \frac{Q_d}{2^{\abs{t}}} + \text{\advantage{$C$,\users}{loE}}
\end{align*}
where $\abs{t}$ is the output length of the loMAC. The running time of $B$ is at most that of A plus the time required to run $Q_e$ many \textnormal{E.enc} encapsulations and $Q_d$ many \textnormal{E.dec} decapsulations. The running time of $C$ is at most that of \adversary. Additionally, $B$ makes at most $Q_e$ many Omac queries and at most $Q_d$ many Ovrf queries and $C$ makes at most $Q_e$ many Oenc queries.
\end{theorem}
\noindent
...
\end{align*}
Next we define game N2-switch-1 in Figure \ref{fig: N2-switch-1 game}. The only difference between this game and game loAE-N2 is the fact that N2-switch-1 uses the uniformly random function $tag$, instead of the loMAC. To define this function we write $\mathit{Func}(\keyspace,\lockspace,\ciphertextspace,\tagspace)$ to denote the set of all functions from key space all functions from key space \keyspace, lock space \lockspace{} and ciphertext space from E.enc \ciphertextspace{} to tag space \tagspace. We define this function specifically as we want the tags resulting from computations in oracle Oenc to match with those in oracle Odec.

\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        \pseudocode[lnstart=-1,linenumbering,head={\textbf{Game} N2-switch-1$_{\adversary,\users}$ }]{
        L \result \emptyset\\
        tag \sample \mathit{Func}(\keyspace_{mac},\lockspace,\messagespace,\tagspace)\\
        \pcfor \user \in [1..N]:\\
        \t \keyinstance_\user \sample \keyspace\\
        \t C_\user \result \invalid\\
        b' \result \adversary\\
    ...
    Key space $\keyspace_{mac}$ is the key space from M.mac.}
...
\noindent
This adversary has the runtime of \adversary{}. For every Oenc query \adversary{} makes, $B$ computes E.enc once, and calls Omac once. For every Odec query \adversary{} makes, $B$ computes E.dec once and calls Ovrf once. Note that, alternatively, $B$ could return 0 if $passed$ is \codetrue{} to avoid having to do E.dec computations. To increase consistency with the other two cases, these computations are still made. Now we can see that $\text{Pr}[\text{loMAC-PRF}_{\text{$B$,\users}}^0 =0]=\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =0]$ as $B$ perfectly simulates game loAE-N2 with $b$ = 0 whenever its own $b$ is 0. In addition, we can see that $\text{Pr}[\text{loMAC-PFR}_{\text{$B$,\users}}^1 =0]=\text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}} =0]$ as $B$ perfectly simulates game N2-switch-1 whenever its own $b$ is 1. As a result we can rewrite our probability to:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &=
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =0]
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
To expand our probability again, we define game N2-switch-2 in Figure \ref{fig: N2-switch-2 game}. The only difference between this game and the first switch game is that game 2 will always return \invalid{} when the Odec query is called. We use this game to expand our probability:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &=
    \text{\advantage{$B$,\users}{loMAC}} 
...
\begin{figure}[H]
    ...
    \end{pchstack}
    \caption{N2-switch-2 game, adversary{} has access to oracles Oenc and Odec. Key space $\keyspace_{mac}$ is the key space from M.mac.}
    \label{fig: N2-switch-2 game}
\end{figure}
(\textbf{todo: argument about switch})
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
We define game N2-switch-3 in Figure \ref{fig: N2-switch-3 game} to expand our probability one last time. Switch game 3 is equal to switch game 2 but always return lazily sampled random bits when the outcome of E.enc is valid. We use this game to expand our probability to:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
...
\begin{figure}[H]
    ...
    \end{pchstack}
    \caption{N2-switch-3 game, adversary{} has access to oracles Oenc and Odec. Key space $\keyspace_{mac}$ is the key space from M.mac.}
...
\begin{align*}
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0].
\end{align*}
By definition, $\text{Pr}[\text{N2-switch-3}_{\text{\adversary,\users}} =0]$ and $\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0]$ are equal, giving our result:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
...
in the real world, the oracle Odec from N2-switch-1 returns \invalid{}, unless the condition on line 26 is \codefalse{} and thus, N2-switch-1 and N2-switch-2 are equal whenever this condition is \codefalse. As \taginstance{} and \taginstance' are the result of the uniformly random function $tag$, where the input of $tag$ may not be equal when computing both, the chance of \taginstance{} and \taginstance' being equal will be $\frac{1}{2^{\abs{t}}}$ for every Odec calls. Combined with a adversary that makes at most $Q_d$ calls to the decryption oracle, we get a chance of $\frac{Q_d}{2^{\abs{t}}}$ for the condition in line 24 to be \codefalse. using this chance, and fact that N2-switch-1 and N2-switch-2 are equal whenever the condition is \codefalse, we can see that $\text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}}^0]$ is bound by $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =0]$ and $\frac{Q_d}{2^{\abs{t}}}$. Consequently $\text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}} =0] - \text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =0] \leq \frac{Q_d}{2^{\abs{t}}}$ holds and we get:

Next, we can rewrite $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =0] - \text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^1 =0]$ into a loE advantage. To do so, we define adversary $C$ against loE in Figure \ref{fig: adversary c N2}. This adversary is playing game loE-IND-\$-CPA (Figure \ref{fig: loE-IND game}), and has access to \adversary. 
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        ...
        \pseudocode[lnstart=5,linenumbering,head={\text{if \adversary{} calls }\textbf{Oracle} Oenc$(\user,\lockinstance,\messageinstance)$}]{
            ...
            L \result L \cup \{\lockinstance\}\\
            \lockinstance_\user \result \lockinstance\\
            \ciphertextinstance' \result \text{Oenc}(\keyinstance_j,\lockinstance_j,\messageinstance)\\
            \taginstance \result tag(\user,\lockinstance_j,\ciphertextinstance')\\
            \ciphertextinstance \result (\ciphertextinstance',\taginstance)\\
            C_\user \result \ciphertextinstance\\
            \pcreturn \ciphertextinstance
        }
    ...
    \end{pchstack}
    \caption{Adversary $C$, has access to \adversary{} and oracle Oenc. Note that $\keyinstance_\user$ is the MAC key in this game.}
    \label{fig: adversary c N2}
\end{figure}
\noindent
The runtime of $C$ is that of $A$, for every Oenc query $A$ makes, $C$ makes $Q_e$ one Oenc query. (\textbf{Question: do we need to include the tag computations in our theorem?}) Adversary $C$ perfectly simulates N2-switch-2, so we get $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =0] = \text{Pr}[\text{loE-IND-\$-CPA}_{\text{$C$,\users}}^0 =0]$ and $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^1 =0] = \text{Pr}[\text{loE-IND-\$-CPA}_{\text{$C$,\users}}^1 =0]$, thus:
...
\begin{itemize}
    \item paper on ideal vs attainable security (if I still want to include that)
    \item paper for the origin of AE security notion where decryption always returns \invalid{} in the ideal world
    \item paper on why ind\$ implies ind lor
    \item paper with other lock implementations (if exist)
    \item paper for the origin of locks (if not GKP)
    \item paper on the origin of ind\$ and ind lor
    \item paper for the origin of hybrid encryption
