...
The idea of "locks", used for our lAE, originates from GKP, where augmentation using locks is proposed as an alternative to key expansion in a multi user setting in order to prevent cythertext collisions between users. Another alternative solution to this problem would be the use of probabilistic encryption. Generic composition of authenticated encryption using probabilistic encryption is explored by Bellare and Namprempre \cite{AC:BelNam00}.
Another alternative solution would be ... and is discussed in ...\textbf{(question: Im not sure if im missing something obvious.)}

The generic composition of authenticated encryption was first studied by Bellare and Namprempre \cite{AC:BelNam00}. The three most common composition modes encrypt-and-MAC, encrypt-then-MAC and MAC-then-encrypt discussed and evaluated using a probabilistic encryption block. NRS further investigates these composition modes using either nonce-based encryption, or IV-based encryption. For IV-based encryption, 8 schemes are proven secure and for nonce based encryption 3 schemes, relating to encrypt-and-MAC, encrypt-then-MAC and MAC-then-encrypt, are proven secure.

As an alternative to generic composition, authenticated encryption can also be composed non-generically.
\textbf{todo: write something for GCM}
\textbf{todo: write something for OCB}

\textbf{(question: Why did you let me look at GCM and OCB only? )}
