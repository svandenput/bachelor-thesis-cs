...
\end{theorem}
Within this theorem, both $Q_e$ and $Q_d$ refer to the total queries the adversary is allowed to make, not the queries per user. To prove this theorem we start by defining game loAE-N2 in Figure \ref{fig: loAE-N2 game}. This game is the game loAE-IND-\$-CCA (Figure \ref{fig: loAE-IND game}), with AE.enc and AE.dec substituted with the N2 algorithms from Figure \ref{fig: N2 calls}.
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
...
\end{align*}
Next we define game N2-switch-1 in Figure \ref{fig: N2-switch-1 game}. The only difference between this game and game loAE-N2 is the fact that N2-switch-1 uses the uniformly random function $tag$, instead of the loMAC. To define this function we write $\mathit{Func}(\keyspace,\lockspace,\messagespace)$ to denote the set of all functions from \keyspace, \lockspace{} and \messagespace{} to all possible tag instances. We define this function specifically as we want the tags resulting from computations in oracle Oenc to match with those in oracle Odec.
...
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
        ...
        \pseudocode[lnstart=7,linenumbering,head={\textbf{Oracle} Oenc$(\user,\lockinstance,\messageinstance)$}]{
            \pcif C_\user \neq \invalid: \pcreturn \invalid\\
            \pcif \lockinstance \in L: \pcreturn \invalid\\
            L \result L \cup \{\lockinstance\}\\
            \lockinstance_\user \result \lockinstance\\
        ...
        \pseudocode[lnstart=19,linenumbering,head={\textbf{Oracle} Odec$(\user,\ciphertextinstance)$}]{
            \pcif C_\user = \invalid: \pcreturn \invalid\\
            \pcif \ciphertextinstance = C_\user: \pcreturn \invalid\\
            (\keyinstance{}1,\keyinstance{}2) \result \keyinstance_j\\
            (\ciphertextinstance',\taginstance) \result \ciphertextinstance\\
...
\begin{align*}
    ...
    \text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0]
\end{align*}
Next, we can rewrite $\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^0 =0] - \text{Pr}[\text{N2-switch-1}_{\text{\adversary,\users}}^0 =0]$ into a loMAC advantage. To do so, we define adversary $B$ against loMAC in Figure \ref{fig: adversary B N2}. This adversary is playing game loMAC-PRF (Figure \ref{fig: loMAC-PRF game}), and has access to \adversary. 
\begin{figure}[H]
    \begin{pchstack}[boxed,center,space=0.5cm]
...
\begin{align*}
    \text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^0 =0] - \text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^1 =0]& = \text{\advantage{$C$,\users}{loE}}
\end{align*}
Finaly, games N2-switch-2 and loAE-N2 are equal in the ideal world, so $\text{Pr}[\text{N2-switch-2}_{\text{\adversary,\users}}^1 =0]
-
\text{Pr}[\text{loAE-N2}_{\text{\adversary,\users}}^1 =0]$ = 0. Filling both of these in gives us our result:
\begin{align*}
    \text{\advantage{$A$,\users}{loAE}}
    &\leq
    \text{\advantage{$B$,\users}{loMAC}} 
